...
//#import "LoginManageViewController.h"
...
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(dismiss)];
    ...
    self.lockView.verifyPwdBlock = ^(NSString *pwd){
        ...
        if(res){//密码一致
            ...
            NSLog(@"您已尝试了%d次了，然并卵",_touchcount);
            if (_touchcount < 5) {

                [self.label showWarnMsg:[NSString stringWithFormat:@"%@,还有%d次机会",CoreLockVerifyErrorPwdTitle,5-_touchcount]];
                _touchcount = _touchcount + 1;
            }else{

//                [UserDefaultsModel deleteUserData];
//
//                        AppDelegate *myDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
//                        UIViewController *tabBarController1 =  [[UINavigationController alloc] initWithRootViewController:[LoginManageViewController new]];
//                        myDelegate.window.rootViewController = tabBarController1;
...
-(void)vcPrepare{
    ...
    if(CoreLockTypeModifyPwd == _type) {//修改密码上的情况
    ...
    if (CoreLockTypeSetPwd == _type) {//设置密码
        //去账户时，登录设置手势密码的返回
//        if (ISINVTRA != 1 && ISINVTRA != 0) {
//            if (ISINVTRA == 2) {//返回账户
//                self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(glockback)];
//            }else if (ISINVTRA == 3){//返回我的资料
//                self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(dismiss)];
//            }
//        }else{
//            self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(backinvtra)];
//        }
//        
//        ISINVTRA = 4;
    }
...
-(void)glockback{
//    AppDelegate *myDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
//    RDVTabBarController *tabbarC = [TabBarController getTabBarController];
//    myDelegate.window.rootViewController = tabbarC;
//    tabbarC.selectedIndex = 2;
}
-(void)backinvtra{
//    [self.navigationController dismissViewControllerAnimated:NO completion:nil];
//    AppDelegate *myDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
//    UINavigationController *nava = (UINavigationController *)myDelegate.tabBarController.selectedViewController;
//    int a = (int)nava.viewControllers.count-2;
//    UIViewController *Vc = nava.viewControllers[a];
//    [nava popToViewController:Vc animated:NO];
}
-(void)dismiss{
    [self dismiss:0];
}
